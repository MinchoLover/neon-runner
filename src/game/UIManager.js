export class UIManager {
  ...
  showLaunchCue(phase = 'ENGINE SYNC', label = 'IGNITION') {
    ...
    window.clearTimeout(this.launchCueTimer);

    if (this.launchPhase) this.launchPhase.textContent = phase;
    if (this.launchLabel) this.launchLabel.textContent = label;

    this.launchSequence.classList.remove('active', 'final');
    void this.launchSequence.offsetWidth;
    this.launchSequence.classList.add('active');

    if (label.includes('LAUNCH') || label === 'GO') {
      this.launchSequence.classList.add('final');
    }

    this.launchCueTimer = window.setTimeout(() => {
      this.launchSequence?.classList.remove('active', 'final');
    }, label.includes('LAUNCH') || label === 'GO' ? 760 : 460);
  }
  ...
  setCountdown(value) {
    ...
    const wide = value === 'IGNITION' || value === 'GO';

    this.panel.innerHTML = `
      <h1 class="countdown-text ${wide ? 'launch-countdown' : ''}">${value}</h1>
      <p>${subtitles[value] ?? 'GET READY'}</p>
    `;
  }
